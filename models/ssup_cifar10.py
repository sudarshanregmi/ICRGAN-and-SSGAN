...
class Discriminator(nn.Module):
    def __init__(self, args, ssup=False):
        super(Discriminator, self).__init__()
        self.channel = args.channel
        self.resnet = args.resnet
        self.num_rotation = args.num_rotation
        self.ssup = args.ssup
        self.spectral_normed = args.spectral_normed

        self.lrelu = nn.LeakyReLU()
        self.relu = nn.ReLU()
        self.conv1 = conv2d(self.channel, 64, kernel_size = 3, stride = 1, padding = 1,
                            spectral_normed = self.spectral_normed)
        self.conv2 = conv2d(64, 128, spectral_normed = self.spectral_normed,
                            padding = 0)
        self.conv3 = conv2d(128, 256, spectral_normed = self.spectral_normed,
                            padding = 0)
        self.conv4 = conv2d(256, 512, spectral_normed = self.spectral_normed,
                            padding = 0)
        self.fully_connect_gan1 = nn.Linear(512, 1)
        self.fully_connect_rot1 = nn.Linear(512, 4)
        self.softmax = nn.Softmax()

        self.re1 = Residual_D(self.channel, 128, spectral_normed = self.spectral_normed,
                            down_sampling = True, is_start = True)
        self.re2 = Residual_D(128, 128, spectral_normed = self.spectral_normed,
                            down_sampling = True)
        self.re3 = Residual_D(128, 128, spectral_normed = self.spectral_normed)
        self.re4 = Residual_D(128, 128, spectral_normed = self.spectral_normed)
    ...
    def forward(self, x):
        if self.resnet == False:
            ...
            conv4 = conv4.view(conv4.size(0)*self.num_rotation, -1)
...
class Generator(nn.Module):
    def __init__(self, args):
        ...
        self.resnet = args.resnet
        self.fully_connect = nn.Linear(self.z_size, s*s*256)
        self.relu = nn.ReLU()
        self.tanh = nn.Tanh()
        ...
        self.conv4 = conv2d(64, self.channel, padding = 1, kernel_size = 3, stride = 1)
        self.conv_res4 = conv2d(256, self.channel, padding = 1, kernel_size = 3, stride = 1)
